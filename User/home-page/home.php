...
<div class="search-box">
  <input type="text" placeholder="店舗名・キーワードで検索" />
</div>
...
<section>
  ...
  <div class="filter-grid" id="category-grid">
    <?php foreach ($categories as $index => $category): ?>
      <button class="filter-card <?= $index >= 6 ? 'hidden' : '' ?>" data-id="<?= $category['category_id'] ?>">
...
<section>
  ...
  <div class="filter-grid" id="area-grid">
    <?php foreach ($areas as $index => $area): ?>
      <button class="filter-card <?= $index >= 6 ? 'hidden' : '' ?>" data-id="<?= $area['area_id'] ?>">
